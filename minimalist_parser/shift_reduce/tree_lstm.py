"""
From https://github.com/unbounce/pytorch-tree-lstm -- JG
...
class TreeLSTM(torch.nn.Module):
    '''PyTorch TreeLSTM model that implements efficient batching. No dropout, since it is a single layer LSTM.
    '''
    ...
    def forward(self, features, node_order, adjacency_list, edge_order):
        ...
        @param features: vector for each node, not contextualised.
        @param node_order: for each node, how many sets of steps it depends on before we can handle this node.
                            (depth of the subtree it dominates).
        @param adjacency_list: encodes edges of tree. (parent, child) pairs,
                                where parent and child are indices of the two lists above. (Not the tree indices!)
        @param edge_order: same as node order, but per edge. (Node order of the parent)
        Takes Tensors encoding node features, a tree node adjacency_list, and the order in which 
        the tree processing should proceed in node_order and edge_order.
        @return 2 vectors:
                        1. contextually encoded node features
                        2. memory state of LSTM
        '''
    ...
    def _run_lstm(self, iteration, h, c, features, node_order, adjacency_list, edge_order):
        ...
        if iteration == 0:
            c[node_mask, :] = (i * u).to(dtype=c.dtype)
        else:
            ...
            child_h_grouped_by_parent = torch.split(fc, child_counts)
            parent_list = [item.sum(0) for item in child_h_grouped_by_parent]

            c_sum = torch.stack(parent_list)
            c[node_mask, :] = (i * u + c_sum).to(dtype=c.dtype)

        h[node_mask, :] = (o * torch.tanh(c[node_mask])).to(dtype=h.dtype)
