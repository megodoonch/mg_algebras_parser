...
from allennlp.training.metrics import ROUGE, BLEU, CategoricalAccuracy
...
# from minimalist_parser.neural.my_bart import BartEncoder

DecoderCacheType = Tuple[Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], ...]


# @Seq2SeqEncoder.register("mego_bart_encoder")
...
@Model.register("mego_bart")
class Bart(Model):
    """
    Based on bart in allennlp models
    ...
    def __init__(
            ...
            model_name: str,
            vocab: Vocabulary,
            # mego: added beam_size and max_steps
            beam_search: Lazy[BeamSearch] = Lazy(BeamSearch, beam_size=1, max_steps=1024),
            indexer: PretrainedTransformerIndexer = None,
            encoder: Seq2SeqEncoder = None,
            **kwargs,  # anything here I can pass from config file
    ):
        ...
        # self._dev_loss = None
        self.predictions_path = "/predictions/predicted.txt"
        print("***** writing predictions to", self.predictions_path, file=sys.stderr)
    ...
    def forward(
        ...
        # MEGO moved this out of training-only if clause
        ...
        # MEGO
        self._compute_accuracy(bart_outputs.logits,
                               cast(torch.LongTensor, target_ids),
                               cast(torch.BoolTensor, target_mask))

        # Compute the loss on the training set if this is a training step,
        # or the validation set if this is a validation step
        ...
        outputs["loss"] = sequence_cross_entropy_with_logits(
            ...
            label_smoothing=0.1,  # The BART paper mentions label smoothing of 0.1 for sequence generation tasks
            average="token",
        )
        ...
        if not self.training:
            ...
            beam_result = self._beam_search.search(
                initial_decoder_ids, initial_state, self.take_step
            )
            ...
            #     predicted_sentence = [self.vocab.get_token_from_index(word.item()) for word in sentence]
            #     print("Top prediction:", " ".join(predicted_sentence))

            # self._rouge(predictions, target_ids)
            # self._bleu(predictions, target_ids)

            outputs["predictions"] = predictions
            # these take up too much room in the logfile
            # outputs["log_probabilities"] = (
            #     beam_result[1].gather(dim=-1, index=max_pred_indices[..., 0]).squeeze(dim=-1)
            # )

            # mego added to write predictions as we go
            # only use this for predicting, not for training
            self.make_output_human_readable(outputs)
            output_parts = self.predictions_path.split("/")
            output_dir = "/".join(output_parts[:-1])
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            with open(self.predictions_path, "a") as f:
                f.write(outputs["predicted_text"][0] + "\n")

        outputs.pop("decoder_logits")
        return outputs
    ...
    def make_output_human_readable(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        # these take up too much room in the log file
        # output_dict.pop("predictions")
